...
from src import PhenotypeCollection
import sys
...
def create_tar(models_dir, pf2acc_f, pt2desc_f, hmm_name, hmm_model_f,  archive_name):
    ...
    config_s = StringIO(config_df.to_csv(sep = "\t"))
    config_tarinfo = tarfile.TarInfo("config.txt")
    config_tarinfo.size = len(config_s.getvalue())

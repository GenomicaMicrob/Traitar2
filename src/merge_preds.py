...
def comb_preds(phypat_dir, phypat_PGL_dir, primary_name, secondary_name, out_dir, k):
    ...
    m.loc[:, list(m1.columns)] = m1
    m.loc[:, list(m2.columns)] = m.loc[:, list(m2.columns)] + m2
    #combine majority votes
    m1_diff_m2 = list(set(m1.columns).difference(set(m2.columns)))
    m2_diff_m1 = list(set(m2.columns).difference(set(m1.columns)))
    m2_inter_m1 = list(set(m2.columns).intersection(set(m1.columns)))
    
    m_maj.loc[:, m1_diff_m2] = m1_maj.loc[:, m1_diff_m2] * 2
    m_maj.loc[:, m2_diff_m1] = m2_maj.loc[:, m2_diff_m1] 
    
    if not len(m2_inter_m1) == 0:
        m_temp = m_maj.loc[:, m2_inter_m1]
        m_temp[(m1_maj.loc[:, m2_inter_m1] == 1) & (m2_maj.loc[:, m2_inter_m1] == 1)] = 3
        m_temp[(m1_maj.loc[:, m2_inter_m1] == 1) & (m2_maj.loc[:, m2_inter_m1] == 0)] = 1
        m_temp[(m1_maj.loc[:, m2_inter_m1] == 0) & (m2_maj.loc[:, m2_inter_m1] == 1)] = 2
        m_maj.loc[:, m2_inter_m1] = m_temp

...
class evaluate:
    ...
    def evaluate(out, gold_standard_f, traitar_pred_f, min_samples, are_pt_ids = True, phenotype_archive = None):
        ...
        if are_pt_ids: 
            ...
            pc = PhenotypeCollection(phenotype_archive)
        ...
        has_enough_samples = gs.apply(
            lambda x: ((x == 0).sum() >= min_samples) and ((x == 1).sum() >= min_samples)
        )
        ...
        pts = [pt for pt in pts if has_enough_samples.loc[pt]]
        if len(pts) == 0:
            sys.exit("No phenotypes have enough positive and negative samples for evaluation")
        #confusion matrix per phenotype
        conf_per_pt = pd.DataFrame(np.zeros(shape = (len(pts), 4)))
        conf_per_pt.index = pts
        #performance measures per phenotype 
        perf_per_pt = pd.DataFrame(np.zeros(shape = (len(pts), 4)))
        ...
        freq_per_pt = pd.concat([(gs.loc[:, pts] > 0).sum(), (gs.loc[:, pts] == 0).sum()], axis = 1)

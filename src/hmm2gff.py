...
def write_hmm_gff(hmmer_file, out_gff_dir, gene_dict, sample, skip_genes, mode,  pt_models, predicted_phenotypes, description = False):
    ...
    with open(hmmer_file, 'r') as hmmer_fo:
        ...
        for l in hmmer_fo:
            ...
            if gene_dict is not None and gid not in gene_dict: 
                if not skip_genes:
                    print(gid, "not found in input orf gff file", file=sys.stderr)
                    if mode == "ncbi":
                        print("entry might be outdated, skipping", file=sys.stderr)
                        continue
                    if mode == "img":
                        print("might be a semicolon ; delimiter within an attribute; sample is %s\n" % sample, file=sys.stderr)
                        continue
                else:
                    continue
            colour = ""
    ...
    if not len(out_table) == 0:
        out_table_df = ps.read_csv(StringIO("".join(out_table)), sep = "\t", header = None)
